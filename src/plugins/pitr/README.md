...
```typescript
import { pitrPlugin } from "sql2/plugins/pitr";

// Install the plugin (creates schema and functions)
await pitrPlugin(sql);
```
...
```typescript
import { enableTracking } from "sql2/plugins/pitr";

await enableTracking(sql, {
  schemaName: "public",
  tableName: "orders",
...
```typescript
import { getRowHistory } from "sql2/plugins/pitr";

const history = await getRowHistory(sql, "public", "orders", { id: 1 });
...
```typescript
import { restoreRow } from "sql2/plugins/pitr";

const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000);
await restoreRow(sql, "public", "orders", { id: 1 }, oneHourAgo);
```
...
const result = await enableTracking(sql, {
  schemaName: "public",
  tableName: "users",
  primaryKeyColumns: ["id"],
...
#### `disableTracking(sql, schemaName, tableName, keepHistory?)`

Disables tracking on a table.

```typescript
await disableTracking(sql, "public", "users", true); // Keep history
await disableTracking(sql, "public", "users", false); // Delete history
```

#### `getTrackedTables(sql)`

Lists all tables with PITR tracking configured.

```typescript
const tables = await getTrackedTables(sql);
// Returns: [{ id, schemaName, tableName, primaryKeyColumns, enabled, auditCount, ... }]
```

### History Queries

#### `getRowHistory(sql, schemaName, tableName, pkValue, limit?)`

Gets the complete change history for a specific row.

```typescript
const history = await getRowHistory(sql, "public", "orders", { id: 1 }, 100);
...
#### `getTableHistory(sql, schemaName, tableName, options?)`

Gets change history for an entire table within a time range.

```typescript
const history = await getTableHistory(sql, "public", "orders", {
...
#### `getRowAt(sql, schemaName, tableName, pkValue, asOf)`
...
#### `getTableAt(sql, schemaName, tableName, asOf)`

Reconstructs the entire table state at a given point in time.

```typescript
const snapshot = await getTableAt(sql, "public", "orders", yesterdayNoon);
// Returns: [{ primaryKeyValue: {...}, rowData: {...} }, ...]
```

### Restore Operations

#### `restoreRow(sql, schemaName, tableName, pkValue, asOf)`

Restores a single row to its state at the given point in time.

```typescript
const result = await restoreRow(sql, "public", "orders", { id: 1 }, oneHourAgo);
// Returns: {
//   success: true,
...
#### `restoreRowsWhere(sql, schemaName, tableName, filter, asOf, dryRun?)`

Restores rows matching a filter condition to their state at a given point in time.

```typescript
import { restoreRowsWhere } from "sql2/plugins/pitr";
...
#### `restoreTable(sql, schemaName, tableName, asOf, dryRun?)`

Restores an entire table to its state at a given point in time.

```typescript
// Preview what would happen
const preview = await restoreTable(sql, "public", "orders", oneHourAgo, true);

// Actually perform the restore
const results = await restoreTable(sql, "public", "orders", oneHourAgo, false);
// Returns: [
...
#### `undoLastChange(sql, schemaName, tableName, pkValue)`

Reverts the most recent change to a specific row.

```typescript
const result = await undoLastChange(sql, "public", "orders", { id: 1 });
// Restores the row to its state just before the last recorded change
```

### Maintenance

#### `pruneHistory(sql, olderThan, schemaName?, tableName?)`

Deletes audit log entries older than the specified date.

```typescript
// Prune all tables
const result = await pruneHistory(sql, thirtyDaysAgo);

// Prune specific schema
await pruneHistory(sql, thirtyDaysAgo, "public");

// Prune specific table
await pruneHistory(sql, thirtyDaysAgo, "public", "orders");
...
```typescript
const stats = await getTableStats(sql, "public", "orders");
// Returns: {
//   totalEntries: 5000,
//   inserts: 1000,
//   updates: 3500,
...
const history = await getRowHistory(sql, "public", "order_items", {
  order_id: 1,
  product_id: 100,
});
...
The PITR plugin records the PostgreSQL transaction ID for every change, enabling powerful transaction-based queries and rollback operations across multiple tables.

#### `getRecentTransactions(sql, limit?)`

Gets a summary of recent transactions with their affected tables.

```typescript
import { getRecentTransactions } from "sql2/plugins/pitr";

const transactions = await getRecentTransactions(sql, 50);
// Returns: [{
//   transactionId: 12345,
//   changedAt: Date,
//   changedBy: "app_user",
...
#### `getTransactionHistory(sql, transactionId)`

Gets all changes that occurred in a specific transaction across all tracked tables.

```typescript
import { getTransactionHistory } from "sql2/plugins/pitr";

const changes = await getTransactionHistory(sql, 12345);
...
//   tableName: "orders",
//   operation: "INSERT",
//   primaryKeyValue: { id: 1 },
//   oldData: null,
//   newData: { id: 1, total: 100 },
//   changedColumns: null,
//   changedAt: Date,
//   changedBy: "app_user"
// }, ...]
...
#### `restoreToTransaction(sql, transactionId, dryRun?)`
...
```typescript
import { restoreToTransaction } from "sql2/plugins/pitr";

// Preview what would happen
const preview = await restoreToTransaction(sql, 12345, true);
console.log(preview);
// [{
//   schemaName: "public",
//   tableName: "orders",
//   operation: "UPDATE",
//   affectedRows: 3,
//   details: "3 rows updated"
// }, ...]

// Actually perform the restore
const results = await restoreToTransaction(sql, 12345, false);
```

#### `undoTransaction(sql, transactionId, dryRun?)`

Undoes all changes from a specific transaction without affecting subsequent transactions. This is useful when you want to revert a specific operation without losing later changes.

```typescript
import { undoTransaction } from "sql2/plugins/pitr";

// Undo just transaction 12345, keeping all later changes
const results = await undoTransaction(sql, 12345, false);
```
...
#### `restoreTablesToTransaction(sql, transactionId, tables, dryRun?)`

Restores only specific tables to their state just before a transaction.

```typescript
import { restoreTablesToTransaction } from "sql2/plugins/pitr";
...
const results = await restoreTable(sql, "public", "orders", beforeMistake);
console.log(
...
const history = await getRowHistory(sql, "public", "users", { id: 42 });
...
if (confirm("Proceed with restore?")) {
  await restoreTable(sql, "public", "orders", oneHourAgo, false);
}
```
...
await undoLastChange(sql, "public", "orders", { id: 123 });
```
...
import {
  ...
  undoTransaction,
} from "sql2/plugins/pitr";

// Find recent transactions
const transactions = await getRecentTransactions(sql, 20);
...
if (badTx) {
  ...
  const changes = await getTransactionHistory(sql, badTx.transactionId);
  console.log(
    `Transaction ${badTx.transactionId} made ${changes.length} changes:`,
  );
  changes.forEach((c) =>
    console.log(`  ${c.operation} on ${c.schemaName}.${c.tableName}`),
  );

  // Option 1: Undo just that transaction (preserves later changes)
  await undoTransaction(sql, badTx.transactionId);

  // Option 2: Restore to before that transaction (reverts all subsequent changes too)
  // await restoreToTransaction(sql, badTx.transactionId);
}
```

### 6. Rolling Back Related Tables Together

```typescript
import { restoreTablesToTransaction } from "sql2/plugins/pitr";

// A transaction affected orders, order_items, and inventory
// But we only want to roll back orders and order_items

const results = await restoreTablesToTransaction(sql, transactionId, [
  { schemaName: "public", tableName: "orders" },
  { schemaName: "public", tableName: "order_items" },
]);
...
results.forEach((r) => {
  console.log(
    `  ${r.schemaName}.${r.tableName}: ${r.operation} (${r.affectedRows} rows)`,
  );
});
```

### 7. Partial Table Restore (Filtered Rows)

```typescript
import { restoreRowsWhere } from "sql2/plugins/pitr";

// Someone accidentally deleted all orders for a specific customer
// Restore only that customer's orders without affecting other rows

const beforeDelete = new Date("2024-01-15T14:30:00Z");
const results = await restoreRowsWhere(
  sql,
  "public",
  "orders",
  { customer_id: 42 }, // Only restore rows matching this filter
  beforeDelete,
);

console.log(
  `Restored ${results.reduce((sum, r) => sum + r.affectedRows, 0)} rows for customer 42`,
);

// You can filter on any tracked column
await restoreRowsWhere(
  sql,
  "public",
  "orders",
  { status: "pending" },
...
1. **Regular pruning** - Set up a scheduled job to prune old entries:
   ...
   await pruneHistory(sql, new Date(Date.now() - 90 * 24 * 60 * 60 * 1000));
   ```
